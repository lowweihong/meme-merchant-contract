...
# chainId=1337
exploit_address = '0xb910CF7518DA19153B26a4aB25c7f9737bdEf48d'#'0x2FC9945177e9e8a5c0129371d59f258eF54E7F83'
...
private_key = "239529679a5ff4f23ebca4c5410a1eccd79bfef1a2d8d32f68b876637a54df00" 
account = w3.eth.account.from_key(private_key)


def get_image_url(token_id):
    try:
        # First try to read from local metadata
        metadata_path = f"metadata/token_{token_id[:8]}.json"
        with open(metadata_path, 'r') as f:
            metadata = json.load(f)
            return metadata['image']
    except FileNotFoundError:
        print(f"Metadata not found for token {token_id[:8]}")
        return None
...
def process_all_users():
    # Get user count
    user_count = deployed_contract.functions.getUserCount().call({'from': '0xDc497AfB3203408EeCBED87E836F3587d882b522'})
    print(f"Total users: {user_count}")
    
    # Process each user's payments
    for i in range(user_count):
        try:
            user_address = deployed_contract.functions.allUsers(i).call({'from': '0xDc497AfB3203408EeCBED87E836F3587d882b522'})
